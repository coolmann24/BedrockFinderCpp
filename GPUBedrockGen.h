...
std::string kernel =
"kernel void hasFormation(global const long* a, global const long* b, global const int* formation, global const int* formation_size, global int* results_ind, global const int* results_max, global int* results, global const int* x_z_offsets)"//max size results
"{"
...
"	{"
"		if(getBedrock(get_global_id(0) + x_z_offsets[0] + formation[i*4], get_global_id(1) + formation[i*4 + 1], get_global_id(2) + x_z_offsets[1] + formation[i*4 + 2], a, b) != formation[i*4 + 3])"//redefine getBedrock for each context
"		{"
"			found = 0;"
"			break;"
"		}"
"	}"
"	if(found == 1 && *results_ind < *results_max)"
"	{"
