...
std::string kernel =
"kernel void hasFormation(global const long* a, global const long* b, global const int* formation, global const int* formation_size, global int* results_ind, global const int* results_max, global int* results)"//max size results
"{"
"	int found = 1;"
"	int i = 0;"
"	for(i=0; i < *formation_size; i++)"
"	{"
"		if(getBedrock(get_global_id(0) + formation[i*4], get_global_id(1) + formation[i*4 + 1], get_global_id(2) + formation[i*4 + 2], a, b) != formation[i*4 + 3])"//redefine getBedrock for each context
"		{"
"			found = 0;"
"			break;"
"		}"
...
"int getBedrock(int x, int y, int z, long* a, long* b)"
"{"
"	if (y == 0) return 1;"
"	if (y < 0 || y > 4) return 0;"
